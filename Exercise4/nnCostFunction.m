...
%Change y with corresponding 1's and 0's
y_matrix = eye(num_labels)(y,:);

%Calculate J (un-regularized)

J = (-1/m) * sum(sum((((y_matrix.*log(h))) + ((1-y_matrix).*log(1-h))),2));

%Calculate J (regularized)

J = J + (lambda/(2*m))*(nn_params'*nn_params - Theta1(:,1)'*Theta1(:,1) - Theta2(:,1)'*Theta2(:,1));
